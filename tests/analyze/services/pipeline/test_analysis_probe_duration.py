...
def test_probe_duration_raises_when_ffprobe_cannot_be_run() -> None:
    ...
    with (
        patch("phaze.services.analysis.subprocess.run", side_effect=FileNotFoundError("no such file: ffprobe")),
...
def test_probe_duration_raises_on_non_json_stdout() -> None:
    ...
    with (
        patch("phaze.services.analysis.subprocess.run", return_value=completed),
