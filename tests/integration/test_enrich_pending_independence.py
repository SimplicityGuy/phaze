...
The milestone thesis made executable: the enrich pending sets (metadata / analyze) must each surface
every not-done, not-in-flight, not-dedup-resolved music/video file **independent of each other** and
**independent of ``FileRecord.state``**. A single file completes both stages in ANY order; after each
partial completion the completed stage's pending set EXCLUDES the file while the not-yet-done stage
STILL INCLUDES it. This dissolves the cross-stage
...
* **Deadlock-detection cell** -- a metadata-done file whose ``state`` was advanced (dual-write
  reality) is STILL in the analyze set. RED against the pre-cutover state-gated code and GREEN
  post-cutover -- the cell that DRIVES the cutover.
...
* **dedup-exclusion** -- a dedup-resolved file is absent from every set.
...
async def _pending_ids(session: AsyncSession) -> dict[str, set[uuid.UUID]]:
    """Return the id set of each enrich pending helper, keyed by stage name."""
    return {
