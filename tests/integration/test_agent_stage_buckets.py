...
_TARGET_DB = require_test_database(SA_DSN, context="agent-stage-bucket integration tests")
...
async def db_session(async_engine: AsyncEngine) -> AsyncGenerator[AsyncSession]:
    """Yield a rolled-back session on the session-scoped schema with the two FK agents."""
