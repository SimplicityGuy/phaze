...
class TestSceneGroupStripping:
    def test_trailing_uppercase_group_is_stripped_and_reported(self) -> None:
        ...
        assert result.event == "Awakenings Festival"

    def test_alnum_group_with_digit_is_stripped(self) -> None:
        result = derive_query("Skrillex - Coachella - 2018.04.15-320-C4.mp3")
    ...
    def test_group_is_stripped_even_when_source_tags_follow_it_in_the_filename(self) -> None:
        ...
        result = derive_query("Carl Cox - Awakenings - 05-08-2021-SBD-0DAY.mp3")
        assert result.scene_group == "0DAY"
        assert result.source_tags == ("SBD",)
        assert result.event == "Awakenings"
    ...
    def test_all_lowercase_trailing_word_is_not_mistaken_for_a_group(self) -> None:
        ...
        result = derive_query("Amelie Lens - Awakenings - 2021-06-12-thegroup.mp3")
    ...
    def test_no_scene_group_present_is_left_none(self) -> None:
        result = derive_query("deadmau5 - Coachella - 2018-04-15.mp3")
        assert result.scene_group is None
...
class TestSourceTagStripping:
    def test_multiple_trailing_source_tags_are_all_collected(self) -> None:
        result = derive_query("Amelie Lens - Awakenings - 2021-06-12-AAC-PSYCON.mp3")
    ...
    def test_bracketed_source_tag_is_dropped_from_query_and_recorded(self) -> None:
        result = derive_query("Charlotte de Witte - Tomorrowland Mainstage [WEB] 2022.mp3")
        assert result.source_tags == ("WEB",)
        assert "WEB" not in result.query
        assert result.event == "Tomorrowland Mainstage"

    def test_source_token_substring_in_a_real_word_is_not_clipped(self) -> None:
        # "Webster" legitimately CONTAINS "web" -- must not be treated as a source tag, since
        # source tokens are matched as WHOLE trailing fields only, never substrings.
        result = derive_query("Webster - Boiler Room - 2020-01-05.mp3")
        assert result.source_tags == ()
        assert result.artist == "Webster"

    def test_quality_bitrate_token_is_recognized_as_a_source_tag(self) -> None:
        result = derive_query("Nina Kraviz - Sonar - 2019-06-15-320.mp3")
        assert result.source_tags == ("320",)
...
class TestDateExtraction:
    ...
    def test_day_over_12_disambiguates_dd_mm_yyyy_without_a_convention_lookup(self) -> None:
        ...
        result = derive_query("Carl Cox - Awakenings - 27-08-2021-SBD.mp3")
    ...
    def test_no_date_present_leaves_both_none(self) -> None:
        result = derive_query("Nina Kraviz - Boiler Room.mp3")
        assert result.date is None
    ...
    def test_track_number_prefix_is_not_mistaken_for_part_of_a_year(self) -> None:
        ...
        result = derive_query("01 - Carl Cox - Awakenings Festival - 2019-08-10-WEB-FLAC-GRVMSTR.mp3")
        assert result.year == 2019
        assert result.artist == "Carl Cox"
...
class TestParentheticalRadioShow:
    def test_bbc_radio_essential_mix_parenthetical_is_dropped_and_flagged(self) -> None:
        result = derive_query("Above & Beyond - Group Therapy (BBC Radio 1 Essential Mix) 2020-03-14.mp3")
        assert result.radio_show is True
        assert "radio" not in result.query.lower()
        assert "essential mix" not in result.query.lower()
        assert result.artist == "Above & Beyond"
        assert result.event == "Group Therapy"
        assert result.date == date(2020, 3, 14)

    def test_broadcast_parenthetical_is_dropped_and_flagged(self) -> None:
        result = derive_query("Carl Cox - Live Broadcast (Radio Show) - 2019-05-01.mp3")
        assert result.radio_show is True
        assert "radio" not in result.query.lower()
    ...
    def test_non_radio_parenthetical_is_kept_not_dropped(self) -> None:
        ...
        result = derive_query("Carl Cox - Awakenings (Boiler Room Stage) - 2019-08-10.mp3")
        assert result.radio_show is False
        assert "boiler room stage" in result.query.lower()
...
class TestMojibakeRepair:
    def test_double_encoded_artist_is_repaired_before_the_rest_of_derivation_runs(self) -> None:
        ...
        result = derive_query("VÃƒÂ¤th - Time Warp - 2017-04-01-WEB.mp3")
        assert result.artist == "Väth"
        assert "Ã" not in result.query

    def test_already_clean_unicode_artist_is_left_alone(self) -> None:
        result = derive_query("Väth - Time Warp - 2017-04-01-WEB.mp3")
        assert result.artist == "Väth"

    def test_mojibake_repair_does_not_disturb_downstream_date_or_group_extraction(self) -> None:
        result = derive_query("VÃƒÂ¤th - Time Warp - 2017-04-01-WEB-GRVMSTR.mp3")
        assert result.artist == "Väth"
...
class TestQueryShapeAndFallback:
    def test_query_is_artist_event_year_in_order(self) -> None:
        result = derive_query("Carl Cox - Awakenings Festival - 2019-08-10-WEB-FLAC-GRVMSTR.mp3")
        assert result.query == "Carl Cox Awakenings Festival 2019"

    def test_returns_a_dataclass_instance(self) -> None:
        assert isinstance(derive_query("Carl Cox - Awakenings - 2019-08-10.mp3"), DerivedQuery)
    ...
    def test_bare_artist_no_event_falls_back_like_the_naive_heuristic(self) -> None:
        result = derive_query("Nina Kraviz.mp3")
        assert result.artist == "Nina Kraviz"
        assert result.event is None
        assert result.query == "Nina Kraviz"
    ...
    def test_live_at_convention_is_still_recognized_without_the_naive_heuristics_rigid_suffix(self) -> None:
        ...
        result = derive_query("deadmau5 - Live @ Tomorrowland 2019.07.20.flac")
        assert result.artist == "deadmau5"
        assert result.event == "Tomorrowland"
...
class TestInternalHeuristicEdgeCases:
    def test_empty_bracket_content_is_a_harmless_no_op(self) -> None:
        result = derive_query("Carl Cox - Awakenings () - 2019-08-10.mp3")
        assert result.radio_show is False
        assert result.event == "Awakenings"
    ...
    def test_calendar_invalid_iso_date_yields_no_date_but_keeps_the_year(self) -> None:
        ...
        result = derive_query("Carl Cox - Awakenings - 2020-02-30.mp3")
        assert result.date is None
        assert result.year == 2020

    def test_single_character_trailing_token_is_too_short_to_be_a_group(self) -> None:
        result = derive_query("Carl Cox - Awakenings - 2019-08-10-X.mp3")
        assert result.scene_group is None
...
_LABELED_SAMPLE: tuple[tuple[str, str, str], ...] = (
    ("01 - Carl Cox - Awakenings Festival - 2019-08-10-WEB-FLAC-GRVMSTR.mp3", "Carl Cox", "Awakenings Festival"),
    ("Skrillex - Coachella - 2018.04.15-320-C4.mp3", "Skrillex", "Coachella"),
    ("Amelie Lens - Awakenings - 2021-06-12-AAC-PSYCON.mp3", "Amelie Lens", "Awakenings"),
    ("Charlotte de Witte - Tomorrowland Mainstage [WEB] 2022.mp3", "Charlotte de Witte", "Tomorrowland Mainstage"),
    ("Above & Beyond - Group Therapy (BBC Radio 1 Essential Mix) 2020-03-14.mp3", "Above & Beyond", "Group Therapy"),
    ("Carl Cox - Awakenings - 27-08-2021-SBD-0DAY.mp3", "Carl Cox", "Awakenings"),
    ("Nina Kraviz - Sonar - 2019-06-15-320.mp3", "Nina Kraviz", "Sonar"),
    ("VÃƒÂ¤th - Time Warp - 2017-04-01-WEB-GRVMSTR.mp3", "Väth", "Time Warp"),
    ("02 - Adam Beyer - Awakenings - 2020-07-19-CDR-TSP.mp3", "Adam Beyer", "Awakenings"),
    ("Carl Cox - Boiler Room (Radio Broadcast) - 2018-11-02.mp3", "Carl Cox", "Boiler Room"),
)
