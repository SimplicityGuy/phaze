...
async def test_fingerprint_progress_returns_counts(client: AsyncClient, session: AsyncSession) -> None:
    ...
    """
    f1 = _make_file()  # failed engine only -> failed
    f2 = _make_file()  # no engine rows -> total only
    f3 = _make_file()  # state says done, no row -> NOT completed
    f4 = _make_file()  # success row -> completed
