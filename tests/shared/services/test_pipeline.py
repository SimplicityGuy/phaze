...
async def test_get_queue_activity_connects_runtime_registered_agent(session: AsyncSession):
    ...
    Regression: ``main.py`` only opens pools for agents present at boot. A compute agent
    registered at runtime (``phaze agents add --kind compute``) has an unopened psycopg
    ...
    router = FakeTaskRouter()
    router.set_counts("nox", queued=3, active=2)
    # k8s-vox models the runtime agent: its count raises until connect() opens the pool.
...
async def test_get_queue_activity_isolates_one_failing_agent(session: AsyncSession):
    ...
    entire metric). Per-agent failure isolation, alongside the existing per-source split.
    """
    await seed_active_agent(session, "nox")
    await seed_active_agent(session, "k8s-vox")
    router = FakeTaskRouter()
