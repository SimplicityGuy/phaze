"""Per-file sidecar for cloud staging, submission, and reconciliation.
...
upload. The control plane presigns the PUT/GET and deletes the object; this
row is the durable record of where the bytes live and what stage the burst is in.

Kubernetes bursts also record the workload name, bounded analysis and node-loss re-drive counters,
admission phase, and inadmissibility alert state. Node loss remains a separate budget because an
infrastructure failure must not consume a file's analysis-failure attempts.
...
class CloudJobStatus(enum.StrEnum):
    ...
    RUNNING = "running"
    SUCCEEDED = "succeeded"
    # An awaiting file carries a cloud_job row
...
class CloudPhase(enum.StrEnum):
    """Kueue admission progression for a Kubernetes cloud burst.
...
class CloudJob(TimestampMixin, Base):
    """One row per file_id tracking an ephemeral cloud burst."""
    ...
    # file_id-scoped staging key in the bucket (KSTAGE-04). D-08: now nullable -- a compute
    ...
    # D-09: the Kueue/Job name stamped at submit; the reconcile cron looks the Job up
    ...
    # D-04: the Kueue admission progression (queued_behind_quota -> admitted -> running ->
    ...
    # D-06: config-derived backend registry id stamped at dispatch (per-backend in-flight
    ...
    # D-01/D-06: records which ``BucketConfig.id`` staged the current object (MKUE-04).
