...
class Stage(enum.StrEnum):
    """The six pipeline stages whose per-file status is derived from output rows."""
...
FAILURE_IS_TERMINAL: dict[Stage, bool] = {Stage.ANALYZE: True, Stage.METADATA: True}
...
def resolve_status(stage: Stage, scalars: Mapping[str, Any]) -> Status:
    ...
    5-way (including ``SKIPPED``) for the two enrich stages; 4-way (no ``SKIPPED``) for the
...
def domain_completed(status_map: Mapping[Stage, Status], stage: Stage) -> bool:
    ...
    Defined ONLY for the two enrich stages (the keys of :data:`FAILURE_IS_TERMINAL`). The downstream
...
def eligible(status_map: Mapping[Stage, Status], stage: Stage, *, has_approved_proposal: bool = False) -> bool:
    ...
    - ``METADATA`` / ``ANALYZE`` (the two enrich stages): eligible iff status NOT in
      ...
      (D-15/D-16). This yields IDENTICAL truth to the prior per-branch dispatch: METADATA
      (``True``) → eligible when ``NOT_STARTED`` OR ``FAILED`` (ELIG-01 independence); ANALYZE
      (``False``, the ONLY enrich carve-out) → eligible iff ``NOT_STARTED`` — a FAILED analyze is
      excluded (ELIG-03 terminal, retry is manual-only; the 44.5K over-enqueue guard). Mirrors
