...
def _free_slots(snapshot: dict[str, BackendSlot]) -> int:
    """Total free slots across the AVAILABLE backends -- the tick's candidate budget.

    Read at two points with two meanings, both off the SAME live snapshot: once BEFORE the walk (the
    first page's LIMIT, and GATE 1 -- zero means a clean no-op tick), and once per page AFTER routing
    it, when ``remaining`` has been decremented by everything this tick staged (the walk's stop test).
    An unavailable backend contributes nothing even when it has capacity.
    """
...
async def _snapshot_backend_slots(backends: Sequence[Backend], session: AsyncSession) -> dict[str, BackendSlot]:
    """Probe EVERY resolved backend ONCE and return the tick's ``{backend_id: BackendSlot}`` snapshot.

    SCHED-02 / Pitfall 1: ``is_available`` (the Kueue cluster probe / compute agent gate / local
    always-up) and ``in_flight_count`` (the D-05 per-backend cloud_job count replacing the retired
...
def _route_candidate(
    ...
    (assume-UTC, the scan_reaper / pipeline_scans convention) so the pure select_backend staleness
    subtraction (now - lane_entered_at) never raises. ``lane_entered_at`` is the awaiting
    ``cloud_job.updated_at`` surfaced by get_cloud_staging_candidates (D-07): the staleness clock lives
    on the sidecar row, NOT file.updated_at, so it survives Phase 90's removal of the dual-written
    file.state. The parked row is never mutated here.
    """
    # Deferred import (Pitfall: keep the module graph acyclic): backend_selection imports backends,
    # which imports THIS module for the push-job key + compute enqueue leg, so a top-level import
    # would close the cycle. Called once per candidate, but an already-imported module is a sys.modules
    # lookup -- immaterial next to the per-candidate _cloud_budget_for round trip that precedes it.
...
async def _stage_candidate_page(
    ...
    its enqueue, so the caller's SINGLE post-loop commit stays the atomic boundary -- this function
    NEVER commits and NEVER rolls back (either would release the advisory lock + row locks and re-open
    ...
    for index, (file, lane_entered_at) in enumerate(page):
        ...
        try:
            ...
            # hold of THIS candidate ONLY -- distinct from the fileserver-vanish stop, which affects
            ...
            # statement error), the caller's safety net rolls the whole tick back so nothing partial is
...
async def _walk_candidate_pages(
    ...
    is what a short page is compared against to detect the end of the candidate set. ``first_page`` is
    that same pair so it composes with :func:`_next_candidate_page`, which returns it in exactly this
    shape; the loop below re-binds one from the other without reshaping either.
...
def _report_tick_outcome(state: _WalkState, free_slots: int, agent_id: str) -> None:
    ...
    which, sustained, is the exact production signature this bead came from: free cloud capacity, a
    non-empty awaiting queue, and ``staged: 0`` forever. One such tick is unremarkable (a full window
    clears on the next); ``_ALL_HELD_WARN_AFTER_TICKS`` in a row is a stalled lane, and the binned
    reasons say which filter is doing it -- ``cloud_attempts_exhausted`` is the permanent one and means
    operator action (raise the local cap, or reset/expire those attempts), not patience.

    Called ONLY on the paths that reached the candidate loop; see :func:`_note_all_held_tick` for why
    the early-return ticks must not touch the streak.
    """
    all_held = state.tally["staged"] == 0 and sum(state.hold_reasons.values()) == state.scanned and state.scanned > 0
...
async def stage_cloud_window(ctx: dict[str, Any]) -> dict[str, int]:
    ...
    :func:`_report_tick_outcome`) but the TRANSACTION is not. This function remains the sole owner of
    ...
    # push-job key + compute enqueue leg (it imports this module), so importing it at module top would
    # close the backends<->drain import cycle. (The selection policy's own deferred import, for the
    # same reason, lives in _route_candidate.)
    ...
    async with ctx["async_session"]() as session:
        ...
        snapshot = await _snapshot_backend_slots(backends, session)
        ...
        limit = _free_slots(snapshot)
        if limit <= 0:
            return {"staged": 0, "skipped": 0}

        # FIFO oldest-first candidates, bounded to the total free slots, row-locked (one transaction).
        # phaze-9sqa: this is the FIRST PAGE of a keyset walk rather than the whole window. It stays
        ...
        # CR-02 safety net (T-50-cron-raise): the candidate walk + the single post-loop commit run under
        # ONE outer guard so an UNEXPECTED raise -- e.g. a Postgres serialization/deadlock surfaced from a
        # session.execute mid-walk (which aborts the txn, so every subsequent statement INCLUDING the final
        # commit raises), or a raise from _cloud_budget_for outside the per-candidate try -- can
        ...
        try:
            ...
            # A poisoned transaction (or any unexpected raise from the pre-dispatch walk body / the commit)
            ...
            # phaze-9sqa: ``state.scanned`` (every row this tick examined across all pages), not the first
            # page's length -- the rollback discards the whole tick, so everything walked is held. It is at
