...
# The two Identify-workspace pieces (RESEARCH "Don't Hand-Roll" key insight): per-row
...
# contract (:mod:`phaze.services.pagination`). The prior implementation was a whole-corpus read --
...
async def get_tracklist_sets_page(
    ...
    total M). Membership and row shape are unchanged by the bound; phaze-1wvb only added the bound.
