...
| Production reference host | **lux** — 125 GB RAM, 14 cores, **47 GB free** on a 98 %-full 1.7 TB NVMe |
...
The audio lives on **nox** at `/mnt/md1/staging` (a 29 TB array, 93 % full), which is the host
bind-mount behind `/data/staging` inside nox's `phaze-agent-worker-*` containers — the path that
appears in `files.original_path` in the live database.

> **Host note for the record.** `/data/staging` exists on **nox** only. `lux` — which runs
> `phaze-api`, `pgbouncer` and the production Postgres — has **no** `/data/staging` and no
> `/mnt/data/staging`; its `phaze-api` container has no such mount, and `/mnt/data` there holds
> only `postgres-dumps`. There is therefore **no** duplicate mount path across the two hosts.
> lux was used for exactly one thing: **read-only aggregate SQL against the production database**
> to draw the sample and to establish the archive's real shape (E2). No benchmark, no write, and
> no audio came from lux.
...
| Copied from | `nox:/mnt/md1/staging` via `rsync`, read-only, 3.9 GB, 102 s |
...
```console
$ # read-only, against lux's production phaze DB
...
Read-only aggregates against the **live production database** on lux (2026-07-26). This closes
...
| **2** | **8 GB RAM is the design point.** An index that must stay graph-resident is the wrong shape for a Raspberry Pi; a disk-paged, int8-quantized, mmap'd, cache-capped, memory-returning IVF is exactly the right shape. | **No.** phaze's production host has **125 GB RAM and 14 cores** (lux). phaze's binding constraint is **disk — 47 GB free on a 98 %-full volume** — which is a *different* constraint that happens to push in a similar direction at window scale. Same conclusion, different reason; do not import the reasoning. |
...
graph resident (lux has 125 GB of RAM) the picture is expected to look like the sub-threshold rows
...
estimates of it. Neither is a pgvector limit — on a host that can hold the graph resident (lux has
...
> **7.7 GB**. Production (lux, **125 GB RAM**) satisfies all four comfortably. The 8.3 GB benchmark
...
### Tier by tier, against the production host's real envelope (lux: 125 GB RAM, **47 GB free disk**)
