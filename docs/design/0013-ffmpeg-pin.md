# ADR-0013 — Unify ffmpeg at 7.1, with the base image as the only version control

| | |
| --- | --- |
| **Status** | Accepted — decided 2026-08-20 |
| **Date** | 2026-08-20 |
| **Bead** | `phaze-b62ri` (rescoped twice; the scrapped 8.1 implementation is preserved at tag `salvage/phaze-b62ri-8.1`) |
...
1. **Do not pin ffmpeg with an apt version.** The images install plain `ffmpeg`. Debian locks the
   upstream MAJOR.MINOR line for the life of a release, so the **base image tag is the version
   control** — and an explicit pin would be *worse than nothing*, because it selects away from
   Debian's security updates (§3).
2. **Move the arm64 agent base from bookworm to trixie.** This is the whole mechanism on that
   image: it is what takes its ffmpeg from 5.1.x to 7.1.x, and its `libav*-dev` with it.
...
| --- | --- | --- |
| Should the bead's 8.1 pin stand, given the wheel links ffmpeg 7.1 and trixie already ships 7.1.5? | *"i think this bead was mis-filed. let's scrap the work entirely and change the scope to pin the CLI to 7.1"* | 2026-08-20 |
| Should the Dockerfiles carry an explicit apt version lock? | *"in all of the Dockerfiles, we don't actually need to do this: ${FFMPEG_APT_VERSION}. Remove that version locking … Each major release of Debian locks the major release of tools installed through apt … not locking ffmpeg would allow minor and patch versions to float. This is fine … all containers are built from the same build time, so it is highly unlikely that one container would end up with a different version of ffmpeg from the other."* | 2026-08-20 |
| Does removing the version locking also revert the arm64 base move? | Keep the base move — the "no change to the Dockerfiles" instruction scoped the version-locking mechanism, not the base image. | 2026-08-20 |
| Should CI move to 7.1 to match the images? | *"leave CI on 8.1.x and the containers on 7.1.5. Yes, this is a discrepancy, but for CI ffmpeg is used only for the audio extract path. this is sufficiently isolated that this discrepancy is fine."* | 2026-08-20 |

**Durable record:** the `phaze-b62ri` bead comment thread. The superseded 8.1 decision
(*"lock to the latest ffmpeg 8.x, same as in github workflows"*) must not be re-derived.
...
introduced a skew that did not previously exist**, which is why that implementation was scrapped.

So the app image changes no version at all. The agent image moves 5.1.x → 7.1.x, and that is what
brings it onto the same line as everything else.

## 3. Why there is no apt version pin — and why one would be worse than nothing

**An explicit apt pin is not merely redundant here — it is actively worse than no pin.** Three
reasons, the first of which is measured:

**1. It pins security updates *out*.** Debian ships ffmpeg security fixes as new Debian revisions
inside the same upstream line, and `apt-get install ffmpeg=<exact>` refuses them. This is not
hypothetical: **bullseye currently carries two ffmpeg versions at once**, and which one you get
depends entirely on whether you pinned —

```
ffmpeg | 7:4.3.9-0+deb11u2 | deb.debian.org/debian-security bullseye-security/main
ffmpeg | 7:4.3.7-0+deb11u1 | deb.debian.org/debian           bullseye/main
candidate: 7:4.3.9-0+deb11u2
```

A pin at `7:4.3.7-0+deb11u1` — the version in the base suite — resolves **away from the security
update**. A pin at `7:4.3.9-0+deb11u2` breaks the build outright the day `u3` lands, and stays
broken until a human edits the string. Unpinned, apt takes the security candidate automatically.

**2. Debian already provides the lock that matters.** A release fixes the upstream MAJOR.MINOR
line for its whole life; only the patch and Debian revision float. Measured across three releases:

| Debian release | ffmpeg | upstream line |
| --- | --- | --- |
| 11 bullseye | `7:4.3.7-0+deb11u1`, `7:4.3.9-0+deb11u2` | **4.3.x** |
| 12 bookworm | `7:5.1.9-0+deb12u1` | **5.1.x** |
| 13 trixie | `7:7.1.5-0+deb13u1` | **7.1.x** |

So `python:3.14-slim` (trixie) *means* ffmpeg 7.1.x and cannot mean anything else. The base tag is
a stronger and cheaper lever than an apt pin, because it moves the CLI **and** the `libav*-dev`
essentia compiles against together — both come from one Debian source package — which is exactly
the property the extract-then-decode seam needs.

**3. It created duplication that Docker cannot single-source.** The pinned shape carried the
version string in two Dockerfiles, which an earlier draft of this ADR called "irreducible" and
guarded with a cross-file test. Removing the pin removes the duplication, and the guard with it.
Cross-image consistency is now structural rather than asserted: both images build from one CI run
against one apt snapshot, and both resolve the same trixie suite.

The residual risk is the one the operator named and accepted: two images built minutes apart could
in principle straddle a Debian security publish and land on different patch revisions of 7.1.x.
Same upstream line, same ABI, and the divergence self-heals on the next build.
...
- A **real decode producing real descriptors** ran on three fixtures; §5 has the numbers and the
  comparison against the bookworm image.
...
is why the release line the base image fixes has to be a deliberate choice rather than an
incidental one.
...
Only two facts in the tree are still assertable, so `tests/agents/deployment/test_ffmpeg_pin.py`
asserts exactly those:

| Guard | Catches |
| --- | --- |
| `test_arm64_agent_base_is_trixie` | The base reverting to bookworm — which silently reverts that image's ffmpeg to 5.1.x and re-opens the app/agent split, with **no version string anywhere in the diff** |
| `test_arm64_agent_stays_on_python_313` | The 3.13 exception being "tidied" to match the repo's 3.14 contract; TF ships no cp314 aarch64 wheel (PR #326) |
| `test_ci_image_ffmpeg_divergence_is_recorded` | The CI/image split becoming undocumented |

Plus `ffmpeg -version` / `ffprobe -version` in each runtime apt layer, which surfaces the resolved
version in the build log.

**The version-pin guards were deleted, not weakened.** The earlier shape asserted that the two
declared pins agreed, that the major had not drifted, that no ffmpeg-family package escaped the
pin, and that no literal was repeated at an install site. With no pin, none of those has anything
to assert. Keeping them as vacuous passes would be worse than having no test, because a green
check reads as coverage. If a test no longer has a claim to make, it should not exist.

One thing worth recording about the deleted guards: while they existed, running them found **two
bugs in themselves** — a scanner that read Dockerfile *comments* containing `apt-get install` as
real install lines, and a `bookworm` check broad enough to have deleted the accurate historical
citation of PR #326. The same comment-blind parsing bug also existed in
`test_agent_image_push_deps.py`, where re-wrapping the apt layer made it report `rsync` and
`openssh-client` as missing when both were installed; that parser was fixed to join line
continuations and is the one change here to a test this bead did not own.

## 7. CI still tests 8.1 — DECIDED: the split stands
