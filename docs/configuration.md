...
`WORKER_JOB_TIMEOUT=600` and `WORKER_MAX_RETRIES=4` are generic role defaults, not the analysis
policy. Every fresh enqueue and every scheduling-ledger replay passes through
`apply_project_job_defaults`, which pins `process_file` to **`timeout=0`**, a heartbeat derived as
**`2 × analysis_stall_timeout_sec`** (`3600` at the `1800` default), and **`retries=2`**. The hook
also replaces a legacy replay carrying the retired `timeout=7200` value, so recovery cannot silently
restore the old wall clock.
...
| Variable                       | Required | Default | Description                                          |
|--------------------------------|----------|---------|------------------------------------------------------|
| `PHAZE_LANE_ANALYZE_CONCURRENCY` (or `lane_analyze_concurrency`) | No | *(host-derived)* | Concurrency of the analyze lane worker (`process_file`; one CPU-bound Essentia `analysis_child` subprocess per active job). **Not a fixed default**: a `default_factory` returns `derive_sizing().concurrency`, i.e. `physical_cores // intra_op_threads` — see [Thread sizing](#thread-sizing--derived-from-the-host-not-configured-phaze-rvcn), which derives both halves together. This matters for memory: peak analyze RSS is *concurrency × per-child-process peak*, so a host with more physical cores runs more Essentia child processes, not larger ones. Set it explicitly to pin the lane (the cloud-agent compose's `worker` service sets `1`). |
...
| `agent_ref` | `compute` | **REQUIRED** — names the compute agent node the rsync push dispatches to; construction **fails fast** (id-tagged `ValueError`) if absent (REG-02, D-13). Not a `kueue` field: a Kueue cluster dispatches by submitting Jobs, never by addressing an agent queue. (`kueue` accepted an optional `agent_ref` between phaze-ifcr and phaze-2u8v.4 purely to key the admin-page dedupe, which is now structural; a leftover key in an existing `backends.toml` is ignored, not an error.) |
...
`[backends.kube]` keys of note: `api_url`, `namespace`, `local_queue` (all **required** on a `kind="kueue"` backend), plus `context` — the **per-cluster kubeconfig context** name (MKUE-01) that selects among N clusters; when omitted the client uses the kubeconfig's current-context. `context` is a plain kubeconfig context name, **not** a secret. Further optional keys: `env_configmap_name` (default `phaze-agent-env`) and `env_secret_name` (default `phaze-agent-token`) — the operator-created ConfigMap/Secret names the submitted Job's pod pulls its agent env and token from (referenced by name only, like `ca_secret_name`); and `active_deadline_seconds` (default **`None` — no deadline**; when set, bounded `gt=0`) — an **opt-in** Job-level wall-clock bound emitted as `spec.activeDeadlineSeconds`. Left unset, the key is absent from the manifest entirely and **nothing kills a running analyze** (phaze-202e). It was a required 3h bound under phaze-1b39; that SIGTERM'd every 2–6 h concert-set analyze at exactly 3h, burned `cloud_submit_max_attempts` per file and stalled the burst lane (incident 2026-07-28). The wedged-pod protection it was reaching for now comes from **pod-state detection** in `reconcile_cloud_jobs`: a pod whose container is waiting in `ImagePullBackOff` / `ErrImagePull` / `InvalidImageName` / `CreateContainerConfigError`, a pod unschedulable past a 15 min scheduling probe, or an un-suspended Job with zero pods past a 15 min probe, is a no-callback terminal — and a **Running pod is never terminalized, at any age**. Setting this key re-arms the incident behaviour on that backend; do not set it as a matter of course. Existing configs carrying it still validate.
...
| `kube_job_cpu_request` | `[backends.kube]` `cpu_request` | Control | `None` | no | CPU resource request stamped on the submitted Job's pod spec (e.g. `2`). Optional. **Validated as a Kubernetes Quantity at config load** (phaze-frq98): a malformed value (`4GB` for `4Gi`, `4K` for `4k`, a stray trailing newline) now refuses to load and names the field, instead of being accepted by phaze and rejected only by a live apiserver one submit round-trip later. **Format only** — no value is judged or rewritten, so `cpu_request = "500"` (500 whole CPUs, if `500m` was meant) is well-formed and still passes; that one surfaces as a Kueue quota rejection. |
| `kube_job_memory_request` | `[backends.kube]` `memory_request` | Control | `None` | no | Memory resource request stamped on the submitted Job's pod spec (e.g. `4Gi`). Optional. **Validated as a Kubernetes Quantity at config load** (phaze-frq98): a malformed value (`4GB` for `4Gi`, `4K` for `4k`, a stray trailing newline) now refuses to load and names the field, instead of being accepted by phaze and rejected only by a live apiserver one submit round-trip later. **Format only** — no value is judged or rewritten, so `memory_request = "4G"` (4 × 10⁹ bytes decimal — about 7% under the `4Gi` almost certainly meant) is well-formed and still passes; that one surfaces as a pod scheduled slightly smaller than intended, never as an error. |
...
- **Runtime non-fatal LocalQueue admission probe (warn + surface).** A correctly-*configured* local-queue can still point at a LocalQueue/ClusterQueue that the cluster admin has mis-set so Kueue never **admits** the Job. That is a *cluster-side* condition phaze cannot detect at startup, so it is handled at runtime, **non-fatally**: the every-minute `reconcile_cloud_jobs` cron maps an `Inadmissible` Workload condition to a warning log and an **Inadmissible** operator-alert card on the pipeline dashboard (it clears when admission recovers). It never crashes the controller — the value was *present* (so startup passed); only the cluster's admission of it is wrong.
...
| `POSTGRES_PASSWORD`| **Yes**  | (none)        | PostgreSQL password for the `postgres` service. Compose fails at parse time if unset (`${POSTGRES_PASSWORD:?POSTGRES_PASSWORD required}` in the service environment) — there is **no** default. |
| `POSTGRES_DB`      | No       | `phaze`       | PostgreSQL database name created on first boot.                            |
| `POSTGRES_BIND_IP` | No       | `127.0.0.1`   | Host interface to bind Postgres `:5432` on (`docker-compose.yml`'s `postgres.ports`). Production overrides to the app-server's LAN IP so off-host agents can connect. **Never set this to `0.0.0.0`** — that exposes the database on every interface. Mirrors `REDIS_BIND_IP`. |
...
- **Postgres password (Compose)** — `POSTGRES_PASSWORD` must be set or `docker compose` aborts at parse time (`${POSTGRES_PASSWORD:?POSTGRES_PASSWORD required}` in `docker-compose.yml`'s `postgres` service).
- **Scan path (agent Compose)** — `SCAN_PATH` must be set or `docker compose -f docker-compose.agent.yml` aborts at parse time. It is enforced on **all five** agent services with `${SCAN_PATH:?SCAN_PATH required}`, mounted `rw` on the meta and drain workers and `ro` everywhere else (phaze-6bkk).
