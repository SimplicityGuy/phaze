...
| FFmpeg (system) | **7.1.5, pinned** — Debian trixie `7:7.1.5-0+deb13u1`, one version string per image (`ARG FFMPEG_APT_VERSION`) | The `ffmpeg`/`ffprobe` **CLI binaries** phaze shells out to: video-container audio extraction (`services/video_audio.py`) and the analysis duration probe (`services/analysis.py::_probe_duration_sec`, D-10) | **Pinned, and unified across every surface** (phaze-b62ri, 2026-08-20). The previous `8.x` claim in this row was wrong for both images: measured, the app image (trixie) served **7.1.5** and the arm64 agent (bookworm) served **5.1.9** — two different majors. 7.1 is the right line to freeze on rather than merely the incumbent, because **the essentia-tensorflow x86_64 wheel statically links ffmpeg 7.1** (libavformat 61.7.100) and that is what decodes audio for analysis; for video the CLI extracts an artifact essentia's libav then decodes, so the two sit across a live producer/consumer seam — the seam that cost 11.5 h in phaze-3ea41. Pinning to 8.x would have *introduced* a skew there. Source is **Debian apt, version-pinned** (not a downloaded build): first-party, security-updated within the `-0+deb13uN` line, and `apt-get install ffmpeg=<version>` fails loudly if the base stops serving it. The arm64 agent base moved **bookworm → trixie** to reach 7.1.5 at all. Held identical across `Dockerfile`, `Dockerfile.agent-arm64` and `Dockerfile.job` by `tests/agents/deployment/test_ffmpeg_pin.py`. **CI still tests 8.1** — a known, recorded divergence with an open decision: see `docs/design/0013-ffmpeg-pin.md` §8. |
### Web UI
| Technology | Version | Purpose | Why Recommended |
|------------|---------|---------|-----------------|
...
# apt-get install -y 'ffmpeg=7:7.1.5-0+deb13u1' chromaprint-tools
# NOTE: the version pin is not optional decoration (phaze-b62ri). Unpinned, this resolved to
# 7.1.5 on trixie and 5.1.9 on bookworm — two majors apart across two images. The arm64 agent
# additionally pins its libav*-dev set to the same string, because it compiles essentia from
# source against them. See docs/design/0013-ffmpeg-pin.md.
...
| FFmpeg (system) 7.1.5 pinned | the essentia-tensorflow wheel's **static** ffmpeg 7.1 (amd64) AND the arm64 agent's from-source essentia, compiled against trixie `libav*-dev` 7.1.5 | This is the compatibility that matters and the reason 7.1 was chosen. **amd64:** the wheel's `_essentia.cpython-314-x86_64-linux-gnu.so` statically links ffmpeg 7.1 — no bundled `libav*.so`, no soname refs, so there is no dynamic seam to repoint; the CLI pin at 7.1.5 puts the shelled-out binary on the same release line as the library that decodes. **arm64:** no cp314 aarch64 wheel exists, so essentia is compiled from source against `libavcodec-dev`/`libavformat-dev`/`libavutil-dev`/`libswresample-dev` — all driven from the *same* `ARG FFMPEG_APT_VERSION` as the CLI, so they cannot skew. Reaching 7.1.5 there required the base move to trixie (bookworm tops out at 5.1.9). **Python stays 3.13 on that image** — TF ships no cp314 aarch64 wheel (dependabot PR #326 proved 3.14 breaks the build); base suite, Python, TF and the pinned essentia commit are one combination, not four independent knobs. |
