...
live gates with `ps aux | awk '$11 ~ /\.venv\/bin\/python$/'`, which matches the **interpreter**;
if three are already running, wait rather than adding a fourth. A fan-out wider than that staggers
its **gates**, not its claims. Do **not** count with `grep pytest` — the `uv` and shell wrappers
carry the venv path in their own arguments, so it counts them too: two seats measured it reporting
**8** and **12** against **4** and **5** real runs on the same evening. It errs safe, but a rule that
is visibly wrong in practice is a rule people stop running.
