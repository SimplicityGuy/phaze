...
- [x] **Phase 93: Console Derived-Status Truthfulness** — the file detail slide-in Stage-Eligibility pills + the left-rail stage badges surface the real derived per-stage status/in-flight counts, reusing the same `stage_status` the Files matrix already renders (CONSOLE-01, CONSOLE-02) (completed 2026-07-15, epic phaze-nawk / PR #260: record-slide-in pills now render the shared `_stage_pill.html` token off `stage_status_case` — the same derivation the Files matrix uses; the rail `<aside>` was missing its Alpine `x-data` root so every badge/count binding was inert, pinning the Analyze badge at a server-rendered `0` regardless of in-flight count — fixed in commit d09b398; browser-UAT verified, phaze-nawk.4. NOT a fix-round 1-2 outcome — this epic landed 2026-07-15/16, well before the 2026-07-28 bughunt rounds; see phaze-pw7v.10)
- [x] **Phase 94: Detail Pop-Out Close/Dismiss Fix** — the X/close control fully dismisses the detail pop-out on BOTH the Agents detail and the Analyze-lane detail (the HTMX-swap / Alpine-global-scope trap that today only removes the X icon); fix once, apply to both (CONSOLE-03) (completed 2026-07-15, epic phaze-nawk / PR #260: `_detail_pane.html`'s `hide()` now clears the swapped wave-2 body (not just hiding the ✕), and the after-swap handler reaches Alpine state via `Alpine.$data(this)` instead of a bare global reference that silently no-opped; verified closed on both the Agents-detail (now an expanded row, `_agent_detail_row.html`) and Analyze-lane-detail surfaces, survives the 5s poll-swap; browser-UAT verified, phaze-nawk.4. NOT a fix-round 1-2 outcome; see phaze-pw7v.10)
- [x] **Phase 95: Analyze-View Browser-Slowdown Investigation & Fix** — diagnose and fix the Analyze workspace severely slowing/hanging the browser (client-side render/poll cost vs the `/pipeline/stats` 200K poll); investigate-then-fix (CONSOLE-04) (completed 2026-07-16, epic phaze-zqvh / PR #264: root cause was the unbounded per-file table — 92,335 rows/~105MB/~5s → bounded active-first working set 13,052 rows/~13.2MB/~2.0s; browser-verified ~4.1s open, flat 31-min soak; DENORM-01 stays deferred, see 95-BASELINE/95-STATS-BUDGET/95-VERIFICATION artifacts)
- [x] **Phase 96: Per-Cluster Kueue Liveness Derivation & Agents-Page Identity** — model each Kueue cluster (host-compute/host-compute-alt) as a live per-cluster ephemeral identity derived from in-flight Kueue workloads (ACTIVE not `NEVER`), reconciling the single generic "k8s burst" lane; the shared derivation core (COMPUTE-01) (completed 2026-07-15, epic phaze-zlv / PR #258: `derive_compute_lane_identities` (`services/agent_liveness.py`) models each Kueue cluster as a live per-cluster identity from in-flight workloads; registry-shadowed never-heartbeating compute `Agent` rows are suppressed so a cluster is never shown twice (once dead as an agent row, once as the generic burst lane); epic-level verification gate phaze-zlv.6 passed, 8/8 children complete. NOT a fix-round 1-2 outcome — round 2's kueue unit (PR #359) hardened submit/reconcile *races* in the same area afterward but did not implement this derivation; see phaze-pw7v.10)
- [x] **Phase 97: Compute-Lane Surfacing — Header Count & Per-File Lane Label** — the header agent count includes every active compute lane, and each file's lane is labeled with its real `backend_id`-derived cluster, killing the stale `☁ A1` label; consumes Phase 96's derivation (COMPUTE-02, COMPUTE-03) (completed 2026-07-15, epic phaze-zlv / PR #258: header `Agents · N` now sums `$store.pipeline.agentOnline + computeLanesActive` (`templates/shell/partials/header.html`); each file's lane badge derives from `CloudJob.backend_id` via `f.lane_kind` (commit c7736a2), the stale `☁ A1` heuristic removed. NOT a fix-round 1-2 outcome; see phaze-pw7v.10)
- [x] **Phase 98: Cloud-Drain Liveness-Gate Investigation & Fix** — diagnose whether the drain dispatch falsely gates on the heartbeat-liveness compute agents never emit (stranding the backlog), fix the gate if the stall is real, and make the Cloud Routing card message reflect real routing state; consumes Phase 96's corrected liveness signal (DRAIN-01, DRAIN-02) (completed 2026-07-15, epic phaze-qtk / PR #257: DRAIN-01 investigated and recorded as COSMETIC/refuted — `KueueBackend.is_available` has no compute-agent/heartbeat dependency, the fileserver gate is ever-checked-in not heartbeat-based, and capacity math explains the observed backlog (phaze-qtk.1 verdict) — collapsing into DRAIN-02 per the phase's own investigate-then-fix contract; the one real defect found (drain snapshot probes lacked the lane-snapshot's `asyncio.wait_for` bound) was fixed alongside the truthful `derive_cloud_hold_reason` Cloud Routing caption (no more hardcoded "held — no compute agent online"), phaze-qtk.2/phaze-613. NOT a fix-round 1-2 outcome; see phaze-pw7v.10)
- [x] **Phase 99: Progress-POST Timeout & Log-Spam Quieting** — give progress POSTs a short connect-timeout + zero retries and demote the progress-path transport-error log to debug, killing the sustained `ConnectTimeout` spam; small and independent (OBS-01) (completed 2026-07-14, PR #252 + regression-guard PR #253, backfilled as epic phaze-ph99: progress POSTs (`services/agent_client.py`) use a short connect-timeout with zero retries, the transport-error log demoted to debug, a regression guard pins the short-timeout/no-retry client, and a dropped POST still never fails the analysis job. NOT a fix-round 1-2 outcome; see phaze-pw7v.10)
- [x] **Phase 100: Human-Friendly Pod Console Logs** — a readable per-job startup banner (filename, source path/origin, target cluster / `backend_id` / bucket, duration, size) + human-phrased step lines, alongside the existing structured JSON logs; small and independent (OBS-02) (completed 2026-07-15, epic phaze-sfbx / PR #259: `job_runner.py` emits a human-readable startup banner (filename, source, target cluster/`backend_id`/bucket, duration, size) plus human-phrased step lines beside the existing structured JSON fields; essentia's own stdout is bracket-framed so it never reads as the app's own logs. NOT a fix-round 1-2 outcome; see phaze-pw7v.10)
...
**Success Criteria** (what must be TRUE):
  1. On the Agents page, each Kueue cluster (host-compute, host-compute-alt) renders ACTIVE while it is running workloads — not perpetually `NEVER` / dead.
...
**Success Criteria** (what must be TRUE):
  ...
  3. The stale `☁ A1` label never appears when no A1 backend is configured (only Kueue host-compute/host-compute-alt + local) — and no A1 lane is re-added.
...
**Success Criteria** (what must be TRUE):
  ...
  3. Prod `alembic_version == '039'` is re-confirmed via the read-only PG probe (`ssh operator@host-prod.lan`, `BEGIN TRANSACTION READ ONLY`) immediately before merge; if not at `039`, the merge holds.
