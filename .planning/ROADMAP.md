...
_Run `/gsd:new-milestone` to scope the next milestone (questioning → research → requirements → roadmap)._
...
| 30. Fix control-plane SAQ queue misrouting | v4.0 | 0/5 | Planned | — |
...
Plans:
- [ ] 30-01-PLAN.md — Routing foundation: named controller queue in lifespan, remove default queue, enqueue-routing helper + active-agent selection
- [ ] 30-02-PLAN.md — Fix pipeline.py (process_file / generate_proposals / extract_file_metadata / fingerprint_file — 8 handlers) + tests
- [ ] 30-03-PLAN.md — Fix tracklists.py (scrape/search/match → controller; scan_live_set → per-agent) + scan-status poll re-targeting + tests
- [ ] 30-04-PLAN.md — Fix legacy /api/v1/scan → ingestion extract_file_metadata per-agent routing + tests
