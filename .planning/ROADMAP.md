...
- [ ] **Phase 67: Backend Registry & Config Model** — declarative `backends:` list (id/kind/rank/cap) as the single source of truth + per-kind discriminated-union validators + `cloud_target`→`backends` back-compat shim + the S3 staging-bucket registry (public/shared vs cluster-specific), config-model-only and behavior-preserving (REG-01..05)
...
**Goal**: Operator can declare the full set of execution backends (and their S3 staging buckets) in config as the single source of truth, while every existing `cloud_target` deploy keeps working unchanged. Behavior-preserving — the shim synthesizes a one-entry registry so nothing observably changes yet.
...
**Success Criteria** (what must be TRUE):
  ...
  3. An existing `cloud_target=a1|k8s|local` deploy with no `backends:` list keeps working unchanged: the shim synthesizes the one-entry registry, `cloud_target=local` resolves to the all-local no-op (never a silently-empty / wedged-backlog registry), and setting both `backends:` and a non-default `cloud_target` fails fast rather than silently picking one.
  4. Operator can declare an S3 staging-bucket registry (each bucket shared/public or cluster-specific, credentials via `<VAR>_FILE`) and assign each Kueue backend its bucket set; the single global S3 config back-compat-shims to a one-entry shared bucket, and a Kueue backend that resolves to an empty/unreachable bucket set fails fast.
