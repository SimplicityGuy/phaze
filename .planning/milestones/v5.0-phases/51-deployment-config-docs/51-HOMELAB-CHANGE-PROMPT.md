...
> `.tf`, the tailnet policy, the host-prod Postgres role) lives in the **homelab** repo and is
...
- drains its per-agent SAQ queue from the **host-prod Postgres** broker (`saq_jobs` only — it has
  **no** `DATABASE_URL` and never touches the app ORM tables, DIST-04),
- receives pushed files via **rsync-over-SSH** from **host-store** (the file server), and
- reaches the host-prod HTTP API (`:8000`) and Redis cache (`:6379`) over the tailnet.
...
3. **Least-privilege `phaze_broker` Postgres role** on host-prod — `saq_jobs` access only (§3).

The feature ships **off by default** (`PHAZE_CLOUD_BURST_ENABLED=false`); after the infra
below is provisioned, the operator flips the toggle and restarts the host-prod control plane (§4).
...
**Networking / security-list note.** Tailscale provides the actual A1↔host-prod / host-store→A1 access
...
A1 (tagged `tag:cloud-agent`) gets **only** what is explicitly granted: `A1 → host-prod` on
`tcp:{5432,6379,8000}` and `host-store → A1` on `tcp:22`, and nothing else.
...
- With **default-deny**, the A1 cannot reach anything else on the tailnet, and nothing else
  can reach the A1 except `host-store:22`.
...
- If `port 22` collides with a Tailscale-SSH policy, use a normal `sshd` — a plain `tcp:22`
  ...
  needed here.
- **Placeholders only** — replace `100.x.x.x` / `100.y.y.y` with the real host-prod/host-store tailnet
  IPs in the homelab repo; never commit real IPs into this spec.

---

## 3. Create the least-privilege phaze_broker Postgres role (host-prod)

The compute agent connects to the host-prod Postgres **only** as the SAQ broker — it dequeues,
...
so the grant is redundant there. Confirm the host-prod Postgres major version in the runbook.
...
Apply in this order (Phase 36 "Step D" precedent; SSH targets `operator@host-store` / `operator@host-prod`).
...
2. **homelab:** apply the tailnet **grants ACL** (A1 ↔ host-prod + host-store → A1) (§2).
3. **homelab (`operator@host-prod` Postgres):** run the `phaze_broker` role SQL — the control plane
...
6. **`operator@host-prod` control plane:** set `PHAZE_CLOUD_BURST_ENABLED=true`; **restart** the
...
- **host-store's `PHAZE_PUSH_KNOWN_HOSTS` must be re-provisioned** with the A1's SSH **host key**
...
- [ ] Tailscale **default-deny** grants ACL applied: A1 → host-prod `tcp:{5432,6379,8000}` + host-store → A1 `tcp:22`, tagged `tag:cloud-agent`
...
- [ ] host-store `PHAZE_PUSH_KNOWN_HOSTS` re-provisioned with the A1 host key
...
tailnet policy, and the host-prod Postgres role.*
