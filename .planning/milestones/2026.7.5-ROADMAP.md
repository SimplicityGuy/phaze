...
**Success Criteria** (what must be TRUE):
  ...
  3. Operator can follow a runbook to provision an OCI Always-Free A1 and a Tailscale ACL scoping the A1 to exactly `host-prod:{5432,6379,8000}` + `host-store→A1:22`, plus a least-privilege Postgres role for the queue broker.
