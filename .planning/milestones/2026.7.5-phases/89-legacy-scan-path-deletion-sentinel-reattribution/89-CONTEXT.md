...
- **D-09:** Single-transaction migration. Order within the txn: (1) `UPDATE files SET agent_id=<target> WHERE agent_id='legacy-application-server'`; (2) `UPDATE scan_batches SET agent_id=<target> WHERE agent_id='legacy-application-server' AND status <> 'live'`; (3) `DELETE FROM scan_batches WHERE agent_id='legacy-application-server' AND status='live'` (the synthetic watcher batch — see D-03); (4) **assert** `COUNT(*)` of remaining `legacy-application-server`-owned rows across both `files` and `scan_batches` `= 0`, else `RAISE` (rolls back the whole txn — the sentinel `DELETE` is never attempted); (5) `DELETE FROM agents WHERE id='legacy-application-server'`.
