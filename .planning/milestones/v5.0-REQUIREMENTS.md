# Requirements Archive: v5.0 Cloud Burst Analysis
...
- [x] **CLOUDDEPLOY-03**: A runbook documents OCI Always-Free A1 provisioning and a Tailscale ACL that scopes the A1 to exactly `host-prod:{5432,6379,8000}` + `host-store→A1:22`, plus a least-privilege Postgres role for the queue broker.
