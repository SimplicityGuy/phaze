...
- **`tests/test_main_lifespan.py`** — a `_patch_saq_lifespan` helper (shared monkeypatches: no-op migrations/ensure_dev_agent, fake engine, an `async_session` whose `execute` is an AsyncMock yielding `.scalars().all()` agents, a controller-queue double with real `str` `name=="controller"` + sync `register_before_enqueue` + async `disconnect`/`info`, and `task_router.queue_for` returning a Wave-0 `FakeQueue("phaze-agent-host-store")`), an `_override_health_session` helper (so `/health`'s `SELECT 1` needs no real Postgres), plus three tests:
  ...
  - `test_saq_queues_assembled_and_reused`: `saq.web.starlette.QUEUES` keys == `{"controller", "phaze-agent-host-store"}`, `QUEUES["controller"] is app.state.controller_queue` (no second pool), and `task_router.queue_for` was called with `"host-store"`.
