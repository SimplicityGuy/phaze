...
- [x] **LEGACY-02**: A data-migration reattributes all historical `legacy-application-server`-owned `files` and `scan_batches` to a designated real `kind='fileserver'` agent (e.g. host-store), with a backfill-verification check.
