...
- **Reused helpers:** `_make_file`, `_persist_files_with_duration([_LONG])`, `_persist_failed_with_duration([_LONG])`, `seed_active_agent(session, "host-store", kind="fileserver")` / `seed_active_agent(session, "cloud", kind="compute")`, `wire_fakes(client)` / `install_fake_queues(client)`, `_drain_background()`, `_LONG`/`_SHORT`.
