...
- [ ] **CLOUDAGENT-02**: A compute agent drains its per-agent SAQ queue and PUTs analysis results over HTTP exactly like a file-server agent, with no access to media or app ORM tables (only the SAQ Postgres broker + cache Redis + HTTP API).
- [ ] **CLOUDAGENT-03**: The Agents admin page distinguishes compute agents (kind badge + liveness + queue depth) so the operator can see available cloud capacity.
