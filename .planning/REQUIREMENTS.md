...
- [ ] **REG-04**: A `cloud_target` (`a1` / `k8s` / `local`) back-compat shim synthesizes an equivalent one-entry `backends:` registry at config load so the existing single-target deploy keeps working with no config rewrite; an explicit `backends:` list takes precedence (with a warning on the ignored legacy key), a resolved-registry line is logged at startup, and a total/fail-fast mapping prevents a silently-empty or wrong registry (`cloud_target=local` maps to the all-local no-op, never a wedged backlog). `cloud_target` is **not** removed this milestone.
- [ ] **REG-05**: Operator can declare an S3 staging-bucket registry — one or more buckets, each with its own endpoint + credentials (via `<VAR>_FILE` secrets) and a scope of either **shared/public** (Internet-reachable, usable by any Kueue cluster) or **cluster-specific** (bound to one Kueue backend) — and assign each Kueue backend the bucket set it stages to (its cluster-specific set, or the shared/public set). Validation is fail-fast (a Kueue backend must resolve to a non-empty, reachable-by-that-cluster bucket set). The existing single global S3 config back-compat-shims to a one-entry shared bucket so current deploys are unchanged. (Config model only — behavior is REG-04-preserving until MKUE-02 consumes it.)
...
| Removing `cloud_target` | The only live deploy depends on it; the shim keeps it working. Deprecate now, remove in a later milestone |
