...
- [ ] **KSTAGE-04**: Each staged object uses a `file_id`-scoped key and is deleted on **every** terminal outcome (success, failure, eviction, re-drive), with a bucket lifecycle TTL as a backstop against orphaned-object leaks.
- [ ] **KSTAGE-05**: S3 endpoint, bucket, addressing style, and credentials are operator-provided config via `_FILE` secrets and work against any S3-compatible backend (`endpoint_url`), not just AWS.
