...
- **Multi-Kueue** — N Kueue clusters from config, each with its own kube config + LocalQueue probe/reconcile, staging to an S3 **bucket registry**: each cluster uses either a shared Internet-reachable ("public") bucket set or its own cluster-specific bucket set (control plane stays the sole S3 importer/presigner; pods credential-free; DIST-01 no-media boundary preserved). Kueue is a category separate from the cloud-compute abstraction — cluster-location-agnostic. *(This generalizes the design doc's original "one shared bucket" decision per operator direction, 2026-07-03.)*
...
---
*Last updated: 2026-07-04 — Phase 69 (Tiered Drain Scheduler, SCHED-01..05) COMPLETE + verified 5/5. Phases 67 (Backend Registry, REG-01..05) and 68 (Backend Protocol + 3 impls, BACK-01..04) already shipped. 69 is the first behavior-changing phase: the drain now dispatches each `AWAITING_CLOUD` file rank-first to the lowest-rank available backend under per-backend caps (count-and-claim under one advisory lock), spilling to the next rank when full/offline and to local (rank 99) when all higher ranks are exhausted; reconcile/recovery are single-owner per kind. Gap-closure plan 69-05 added `FileState.LOCAL_ANALYZING` to remove a locally-spilled file from the drain-candidate set (closes a code-review/verifier CR-01 cross-backend double-dispatch blocker). WR-02 (require ≥1 local backend when cloud enabled) + WR-03 (compute reconcile no-op → zero-owner recovery) deferred to Phase 70. Milestone **2026.7.1 Multi-Cloud Backends** in progress (phases 67-71; design on `main` via PR #182). Version is provisional CalVer (`2026.7.1`), finalized at release. Prior milestone 2026.7.0 Engineering Improvements SHIPPED + archived 2026-07-03.*
