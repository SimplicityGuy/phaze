...
2. **CUT-02 — Full dead-code cutover.** Legacy routes ALREADY redirect into the shell (Phase 57
   SHELL-05), so this is pure **deletion** of the unreachable wrappers + their dead `return ...list.html`
   render tails + `pipeline.py`'s fully-dead dashboard branch + the orphaned partial cascade. The live
   per-router HX branches (shell pagination/filter/sort fragments) are KEPT — see corrected D-03b. Drain
   the dead-template guard `_ALLOWLIST` to empty; guard stays green. "No orphaned dead code."
...
- **D-03b (CORRECTED 2026-07-01 — the earlier "strip HX branches" framing was premised on a factual error):** The per-router in-page HX branches are **LIVE, not dead** — verified against source (`proposals.py:164-165` renders `proposals/partials/proposal_content.html`, and identically `tags.py`/`cue.py`/`tracklists.py`/`duplicates.py`). They serve the shell workspaces' in-page pagination/filter/sort fragments (the `# left intact so the app stays usable (D-01)` code comment + RESEARCH's "Partials that STAY" list confirm this). **KEEP them.** For these 5 routers the ONLY dead code is the final unreachable `return ...list.html` tail (e.g. `proposals.py:167`) — delete just that line. The one genuinely-dead HX branch is `pipeline.py`'s (→ `dashboard.html`, which has no live shell fragment) — that whole render path is dead and is removed. **Definition of done:** delete the 8 wrappers + their dead `return ...list.html`/dashboard tails + the orphaned partial cascade (D-03a's 6 partials + the 6 remaining allowlisted `search/*` partials) + drain `_ALLOWLIST` to `frozenset()`; `test_no_orphan_templates` green; closure logic untouched. **Rejected (and now known to be wrong):** stripping the live per-router HX branches — verified to break live shell pagination/filter/sort (a functional regression violating REQUIREMENTS.md line 82).
