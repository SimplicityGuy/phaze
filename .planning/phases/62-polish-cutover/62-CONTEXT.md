...
2. **CUT-02 — Full dead-code cutover.** Supersede the still-rendered legacy full-page routes,
   then delete the orphaned templates/routers/partials/JS. Drain the dead-template guard
   `_ALLOWLIST` to empty; guard stays green. "No orphaned dead code."
...
- **D-03: Full purge — supersede legacy full-page routes, then delete.** The legacy top-level GET routes still render full pages via `TemplateResponse` (`proposals.py:167` `proposals/list.html`, `tracklists.py:160`, `tags.py:226`, `cue.py:236`, `duplicates.py:113`, `preview.py:57` `preview/tree.html`, `execution.py:375` `execution/audit_log.html`). Convert each non-HX top-level GET to a **shell redirect** into its corresponding `/s/<stage>` state — exactly the pattern `/search` already uses (`search.py`: non-HX → `RedirectResponse("/?palette=1")`, HX branch kept). Then delete the now-orphaned page wrappers + any partials/JS they alone referenced. Empty `_ALLOWLIST`; guard green. This realizes CUT-02's "dead templates, routers, and partials … removed once superseded (no orphaned dead code)." **Rejected:** minimal (delete only the 7 allowlisted, leave legacy pages reachable) — that leaves old-UI code live, failing CUT-02's intent.
...
- **D-05: Supersession must be verified before deletion, per legacy page.** For each legacy page, confirm its shell `/s/<stage>` workspace actually covers the page's function before deleting it, so no operator-visible capability is lost in the cutover. Expected mapping (planner/researcher to confirm): `/proposals`→`/s/propose`, `/tracklists`→`/s/tracklist`, `/tags`→`/s/tagwrite`, `/cue`→`/s/cue`, `/duplicates`→`/s/dedupe`, `/preview`→ Move/destination workspace (`/s/move`). If a shell workspace is missing a feature the legacy page had, that is a supersession gap — surface it, do NOT silently drop the capability (and do NOT add new capability to close it — that would be scope creep for a later phase).
...
- `.planning/ROADMAP.md` § "Phase 62: Polish & cutover" (line 48-49) — Goal + 4 Success Criteria (keyboard rail + palette with visible focus and skip link; dead-template guard green; docs/README describe new IA; rail collapses to icons at narrow widths).
