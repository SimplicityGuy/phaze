...
- **D-01:** **Hybrid — GitHub job matrix over buckets is the PRIMARY parallelism; `pytest-xdist` (`-n auto`) is added ONLY on buckets verified to touch no database.** The matrix gives CI-01 its literal "independently-runnable buckets" (each bucket is a separately visible CI job/check); free runner-minutes on the public repo make the ~9-job fan-out cheap. **Revised after Phase 63 research (Q-B):** 55% of test files share one `phaze_test` DB via `create_all`/`drop_all` fixtures, so `-n auto` inside a DB-touching bucket races on schema create/drop. Therefore DB buckets run **serial**; xdist is applied only where a bucket is confirmed DB-free. No shared-DB-fixture rework (that would exceed the CI-only, no-behavior-change scope).
...
- **D-05:** **Shared helpers and the root `conftest.py` stay at `tests/` root** — the entire stay-at-root allowlist is exactly these four: `conftest.py`, `_queue_fakes.py`, `_route_introspection.py`, `kube_fakes.py`. pytest conftest scoping is hierarchical, so the root conftest still applies to every bucket subdir, and absolute imports (`from tests._queue_fakes import …`) keep working. **Every collected test file relocates into a bucket dir** — that means both pytest collection globs, `test_*.py` AND `*_test.py` (e.g. `_queue_fakes_test.py`), not just `test_*.py`.
- **D-06:** Add a **partition-guard test** asserting every collected test file (both `test_*.py` and `*_test.py`) resides under a known bucket directory (fails CI if a new test file is added outside the bucket dirs), closing the "silently unbucketed → dropped from coverage" gap. With directory-based buckets this guard is a path check, not a marker check. The guard must cover both globs so a future `*_test.py` reintroduction is structurally caught.

### Integration tests / DB services placement
- **D-07:** **Every bucket job provisions postgres+redis service containers** — reusing the proven green service block in `.github/workflows/tests.yml`. **Revised after Phase 63 research (Q-A):** the original "dedicated integration job, unit buckets service-free" is impractical because 117/212 test files (55%) consume DB fixtures and are auto-marked `integration`, spread across most workflow-step buckets — isolating them into one job would defeat the per-step partition (CI-01). Uniform service provisioning keeps every workflow-step bucket self-contained; each job pays ~10-20s container startup (acceptable on free public-repo runners). The `integration` bucket still exists as one of the buckets; its `.coverage` and every other bucket's fold into the combined report.
